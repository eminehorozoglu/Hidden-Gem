...
JOIN menu ON menu.id = business.menu_id WHERE business.id = $1 `,
  ...
  const commentdata = await db.query(
    `SELECT reviews.id, reviews.date, reviews.username, reviews.comment,reviews.business_id,business.business_address FROM reviews  
  ...
  return (
    <div className="flex flex-col items-center gap-4 w-full bg-white m-6">
      ...
      <Choise
        ...
        mapsKey={mapsKey}
        placeId={wrangleData[0].place_id}
